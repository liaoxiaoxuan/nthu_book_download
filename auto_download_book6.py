...
options.headless = True #背景執行 #隱藏 #畫面
...
def start():
    driver.get('https://nthu.primo.exlibrisgroup.com/discovery/fulldisplay?docid=alma990002609730206774&context=L&vid=886UST_NTHU:886UST_NTHU&lang=zh-tw&search_scope=MyInst_and_CI&adaptor=Local%20Search%20Engine') #首頁搜尋
    #search = driver.find_element_by_css_selector('input#sw') #send_keys：關鍵字
    #search.send_keys('宋诗 融通与开拓') # send_keys：關鍵字
    #search.submit()
    sleep(5)
    driver.find_element_by_xpath(                                                                                                      #this index                    
        '/html/body/primo-explore/div/prm-full-view-page/prm-full-view-cont/md-content/div[2]/prm-full-view/div/div/div/div[1]/div/div[4]/div/prm-full-view-service-container/div[2]/div/prm-alma-viewit/prm-alma-viewit-items/md-list/div/md-list-item/div[1]/div[1]/div/p/span[2]'
...
for i in range(resume_index,total):
    try:
        ...
        window_ori = driver.window_handles[0]
        url = driver.current_url
        driver.close()
        driver.switch_to.window(window_ori)
        #driver.execute_script("window.open(url)")
        #url = url.replace('http://img.chinamaxx.net.nthulib-oc.nthu.edu.tw/n/d0b90f50479ad212b3e38e247ef6ab8dMC301282372009/img0/', 
        #                  'http://bpng1.5read.com/image/ss2jpg.dll?did=bf1&pid=')
        #print(url)
        #input()
        '''
